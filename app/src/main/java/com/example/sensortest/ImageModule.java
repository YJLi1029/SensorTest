...
public class ImageModule implements View.OnTouchListener{
    ...
    ImageModule(Activity activity) {
        ...
        // Load map image
        InputStream inputStream; //파일에서 데이터를 읽어올 떄 사용
        ...
        options.inJustDecodeBounds = false; // 이젠 이미지 바운드만 읽어오는 것이 아니라 전체를 다 읽어옴.
        options.inSampleSize = 2; // 이미지 사이즈를 2배로 압축
    ...
    // 멀티터치 발생 시 두 손가락 사이 간격, 중심 등을 계산
